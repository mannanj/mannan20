...
export function TurnstileCheck({ onPass, testIdPrefix }: TurnstileCheckProps) {
  ...
  useEffect(() => {
    ...
    const fallbackTimer = window.setTimeout(onPass, TURNSTILE_FALLBACK_TIMEOUT_MS);
    return () => window.clearTimeout(fallbackTimer);
  }, [status, turnstileAvailability, onPass]);
    ...
    >
      ...
      {status === 'verifying' ? (
        <>
          ...
          <p data-testid={`${testIdPrefix}-feedback`} style={{ margin: 0, fontSize: '13px', color: 'rgba(255,255,255,0.5)' }}>
            {VERIFYING_TEXT}
          </p>
        </>
      ) : (
        <p data-testid={`${testIdPrefix}-feedback`} style={{ margin: 0, fontSize: '13px', color: 'rgba(239,68,68,0.7)' }}>
          {TURNSTILE_FAIL_TEXT}
        </p>
      )}
