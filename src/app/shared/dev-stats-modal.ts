...
import { selectDevStatsModalOpen, selectDevStatsActiveTab, selectDevCommits, selectTasks } from '../store/app.selectors';
import { toggleDevStatsModal, setDevStatsTab } from '../store/app.actions';

@Component({
  selector: 'dev-stats-modal',
  imports: [Modal, TasksContainer, CommitsGrid, ServicesPlaceholderIcon],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <modal [isOpen]="modalOpen()" [widthStyle]="'large'" (close)="toggleModal()">
...
export class DevStatsModal {
  private store = inject(Store);

  protected modalOpen = toSignal(this.store.select(selectDevStatsModalOpen), { initialValue: false });
  protected activeTab = toSignal(this.store.select(selectDevStatsActiveTab), { initialValue: 'commits' as 'commits' | 'services' | 'tasks' });
