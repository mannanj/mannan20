...
import { loadDevCommitsSuccess } from '../store/app.actions';
import { DevCommit } from '../models/models';
...
export class DevStats implements OnInit {
  private store = inject(Store);
  private http = inject(HttpClient);

  protected showDevStats = signal(false);
  protected isModalOpen = signal(false);
  protected devCommits$ = this.store.select(selectDevCommits);

  ngOnInit() {
    this.showDevStats.set(isDevMode());

    if (this.showDevStats()) {
      this.http.get<DevCommit[]>('/assets/dev-commits.json').subscribe({
        next: (commits) => {
          this.store.dispatch(loadDevCommitsSuccess({ commits }));
        },
        error: (err) => {
          console.error('Failed to load dev commits:', err);
        }
      });
    }
  }
