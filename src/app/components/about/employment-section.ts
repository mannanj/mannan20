...
export class EmploymentSection {
  ...
  jobs$: Observable<ProfileItem[] | undefined> = this.store.select(selectJobs);
  visibleJobs$: Observable<ProfileItem[]> = this.jobs$.pipe(
    map(jobs => jobs?.slice(0, this.jobsToShow) || [])
  );
  totalJobs$: Observable<number> = this.jobs$.pipe(
